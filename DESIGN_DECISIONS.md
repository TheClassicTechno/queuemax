...
**Behavior:** Format `messageID:queueEpoch:deliveryAttemptSeq`. `deliveryAttemptSeq` is an in-memory counter incremented on every (re)delivery; `queueEpoch` is a random value generated once when the `Queue` is constructed (i.e. once per process start). ACK is honored only if the presented epoch matches the queue's current epoch *and* the presented attempt-seq matches the message's current attempt-seq, checked under the same per-queue lock as lease-expiry evaluation — expiry is lazy/logical, evaluated only under that lock, never an eager out-of-band mutation racing ACK. A late-but-still-current ACK wins over expiry as long as no newer `receive()` has already superseded it.
**Rationale:** Resolves the ACK-vs-lease-expiry race identified in the Phase 0 review; directly satisfies the invariant that a stale receipt handle cannot ACK a newer delivery. The epoch component was added after a Phase 6 test caught a real collision: `deliveryAttemptSeq` alone restarts from the same small integers on every restart (the ledger is never persisted, #9), so a genuinely pre-restart handle could otherwise collide with, and incorrectly ACK, an unrelated post-restart delivery of the same message ID.
**Tradeoff:** Requires the expiry check and ACK check to be genuinely serialized through one lock — covered by a dedicated concurrent test. The epoch is process-lifetime-random, not cryptographically unguessable — acceptable since it defends against accidental cross-restart collision, not an adversarial client.
